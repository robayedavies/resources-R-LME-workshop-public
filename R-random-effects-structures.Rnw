...
\begin{itemize}
  ...
  \item<2->
	Building random slopes into your model may reveal that there is quite a bit of variation in some effects -- enough random variation that fixed effects are not significant -- though that variation may be of interest}	
	\item<3->
