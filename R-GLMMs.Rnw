...
\title
{PSYC 402 -- advanced statistics -- Class Seven}
...
\begin{frame}{\emph{Targets for Class Seven}}{Ideas and skills -- foundations for later work}
  \begin{enumerate}
  \item
...
\begin{frame}{Understand the reasons for using Generalized Linear Mixed-effects models (GLMMs) to analyze discrete outcome variables}{Discrete outcome variables are very common}
  \begin{itemize}
  \item<1->
...
\begin{frame}{We can think of logistic models as working like linear models with log-odds outcomes}
\begin{equation}
...
%    \column{.4\textwidth}
%      \begin{figure}     
%       \includegraphics[scale=0.25]{jaeger-2008-fig-2-logit-probability-space}
%       \caption{\footnotesize{Jaeger (2008) small changes on the x-axis around p = .5 lead to larger differences in probabilities}} 
%       % compared to the same change on the x-axis closer to 0 or 1
%      \end{figure} 
% \end{columns}     
\end{frame}
...
\begin{frame}{What does it tell us that the predicted probability of a categorical outcome takes a sigmoid curve shape?}
%{What does the curve tell us?}
\begin{itemize}
\item<1->
Changes in predictors are associated with larger differences in probability when $p \sim .5$ where slope is steepest (Figure 2, right)
\item<2->
Variance in sample proportions are also greatest around p = .5 so see bigger differences there but less importance?
\end{itemize}
 \begin{columns}[T]
  \column{.5\textwidth}  
     \begin{figure}     
      \includegraphics[scale=0.15]{jaeger-2008-fig-1-variance-prop-mean}
      \caption{\footnotesize{Jaeger (2008; Fig.1) variance of sample proportion dependent on probability response is correct}}
      % -- greatest at $p=.5$}
     \end{figure}
% -- jaeger 2008 p3 --  the variance of the sample proportions will be highest for p = .5 (the product of n numbers x that add up to 1 is highest if x1=. . .=xn) and will decrease symmetrically as we approach 0 or 1. This is illustrated in Fig. 1. Note that the shape of the curve and the location of its maximum are determined by p alone.
% -- Also, as can be see in Fig. 1, differences close to 0.5 will matter less than differences closer to 0 or 1. 
   \column{.5\textwidth}
     \begin{figure}     
      \includegraphics[scale=0.25]{jaeger-2008-fig-2-logit-probability-space}
      \caption{\footnotesize{Jaeger (2008; Fig.2) small changes on the x-axis around $p = .5$ lead to large decreases or increases in probabilities compared to $p \sim 0 or 1$}}
      % the same change on the x-axis closer to 0 or 1}}
     \end{figure}
% -- jaeger 2008 p4 -- 
% -- what if we transformed proportions into a space that is not bounded by 0 and 1 and that captures the fact that, in real binomially distributed data, a change in proportions around 0.5 usually corresponds to a smaller change in the predictors than the same change in proportions close to 0 or 1 (i.e. the relation between the predictors and propor- tions is nonlinear; cf. Agresti, 2002: 122)? Consider odds.
% -- if we transform proportions to odds then we get an outcome that ranges from 0 to positive infinity, o = \frac{p}{1-p}
% -- differences in odds are usually described multiplicatively eg one set of odds is x times greater or smaller than another set of odds
% -- we can describe the odds of an outcome as a product of coefficients raised to the power of the respective predictive values \frac{p}{1-p} = \beta_0*\beta_1^{X_1} ...
% -- by taking the natural log of odds, we can turn the model back into a linear combination ln\frac{p}{1-p} = ln\beta_0 + ln\beta_1^{X_1} ...
% -- because ln\beta_0 + ln\beta_1 + ... are constants, we can substitute \beta_0 + \beta_1 for them so ln\frac{p}{1-p} = logitp = \beta_0 + \beta_1X_1 ...
% -- the logit is centred on 0 ie logitp = -logit(1-p) corresponding to a probability of .5
% -- the logit ranges from negative to positive infinity
% -- the logit maps points in probability space into points in logit space -- in probability space the linear relationship that we see in logit space is gone
% -- we can understand that this will be because p = \frac{e^{x'\beta}}{1 + e^{x'\beta}} = \frac{1}{1 + e^{-x'\beta}} ie probabilities are calculated as equal to the natural log to the power of the effects divided by 1 plus the same
% -- can run \frac{1}{1 + e^{-x'\beta}} with some fake numbers for {-x'\beta} and see indeed that differences in the resulting probabilities are larger if p = .5 compared to p nearer to extremes even if differences in x are the same
% 1/(1+exp(-0.25))
% 1/(1+exp(0.25))
% 
% 1/(1+exp(-.25)) - 1/(1+exp(.25))
% 
% 1/(1+exp(-1))
% 1/(1+exp(-.5))
% 
...
% 1/(1+exp(-1))
% 
% 1/(1+exp(-1.5)) - 1/(1+exp(-1))
% -- jaeger 2008 p4 -- 
% -- Logit models capture the fact that differences in proba- bilities around p = .5 matter less than the same changes close to 0 or 1. This is illustrated in Fig. 2, where the left panel shows a hypothetical linear effect of a predictor x in logit space (y = 􏰆3 + 0.2x), and the right panel shows the same effect in probability space. As can be seen in the right panel, small changes on the x-axis around p = .5 (i.e. x=15 since 0=􏰆3+0.2*15=logit(0.5)) lead to large decreases or increases in probabilities compared to the same change on the x-axis closer to 0 or 1.  
% -- differences in probabilities around p = .5 matter less than the same changes close to 0 or 1: why?
% -- when we look at the probability curve we can see that for constant change in x the probability moves a lot around p = .5 but less towards the extremes, 0 or 1 
% -- how can that mean that differences in probabilities around p = .5 matter less?
% -- we could say that differences in probabilities around p = .5 are associated with smaller changes in X
% -- we also know that variance in probabilities are greatest around p = .5 so we might see bigger differences in probability but those differences are less meaningful -- they also matter less in that sense
% -- baguley p 678 -- if consider the sigmoidal curve (y = probability) produced by the inverse of the logistic function, for values close to p = .5 the slope of the predictor expressed on a probability scale is around .25 times the coefficient on the logit scale
% -- we can see this if we compare the slopes of the jaeger figure 2 logit-scale vs. probability-scale depictions of the effect of x on categorical outcome y 
% -- the slope on the probability scale is clearly steeper than the slope on the logit scale
\end{columns}     
\end{frame}

% -- what makes it difficult for me is the intuition that differences in probability around p = .5 matter more than the same changes closer to 0 or 1
% -- the sigmoid curve of the probability-scale graph of the x effect means that bigger changes in x are required to get the same differences in probabilities as you approach the extremes of probability, 0 or 1
% -- as jeager says "...in real binomially distributed data, a change in proportions around .5 usually corresponds to a smaller change in the predictors than the same change in proportions closer to 0 or 1 (i.e. the relation between the predictors and proportions is nonlinear)..."
% -- changes in probability considered alone cannot matter more or less depending on where in the 0-1 probability range you are
% -- what we can have is bigger changes in probability associated with smaller changes in the predictor around p = .5 compared to the extremes of the 0-1 probability range
% -- this will mean that 

% -- see also jaeger 2008 p8 -- One reason why the arcsine transformation is unreli- able for such data becomes apparent once we compare the plots of logit and arcsine transformed proportions. Fig. 4 shows the slope (1st derivative) and curvature (2nd derivative) of the two transformations. Both trans- formations have a saddle point at p = .5, but for all p 61⁄4 .5 the slope of the logit is always higher than the slope of the arcsine-square-root. The absolute curvature (the change in the slope) is also larger. In other words, as one moves away from p = .5, a change in probability p1 to p2 corresponds to more of a change in log-odds than to a change in arcsine transformed probabilities. means that, compared to the logit, the arcsine transfor- mation underestimates changes in probability more the closer they are to 0 or 1.

% -- so teach what I understand -- that changes in x associated with bigger changes in probability around p = .5, which could mean effects are bigger, but variance also larger there so maybe relatively smaller compared to variance

\begin{frame}{To illustrate GLMMs we use the word learning data set}{Padraic Monaghan, me and colleagues examined the accuracy of responses in a word learning study: noun-verb-learning-study.csv}
...
\begin{frame}[fragile]{Add effect of experimental condition}
\footnotesize{\begin{verbatim}
...
\begin{frame}[fragile]{Add effect of experimental condition and block}{It might be sensible to add both main effects in one step}
\footnotesize{\begin{verbatim}
...
\begin{frame}[fragile]{Extreme correlations (near 0 or 1) between random effects on intercepts and on slopes of fixed effects suggest the level
of complexity in the model cannot really be justified}{Note also that the variances for the random effects of items on the slopes of the block effect are very small}
\begin{itemize}
  \item<1->
